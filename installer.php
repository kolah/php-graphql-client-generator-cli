...

{
	...
	$url = 'https://kolah.github.io/php-graphql-client-generator-cli/bin/gql2php.phar';
	$pubkeyUrl = 'https://kolah.github.io/php-graphql-client-generator-cli/bin/gql2php.phar.pubkey';
	
	$filename = 'gql2php.phar';
	$pubkeyFilename = 'gql2php.phar.pubkey';
	
    file_put_contents($filename, file_get_contents($url));
    file_put_contents($pubkeyFilename, file_get_contents($pubkeyUrl));
		
    echo " - Downloading checksum file...$n";
    list($checksum, $checksumFilename) = explode(' ', file_get_contents('https://kolah.github.io/php-graphql-client-generator-cli/bin/gql2php.phar.version'));

    echo " - Checking file checksum...$n";

    if ($checksum !== sha1_file($filename)) {
        unlink($filename);
        unlink($pubkeyFilename);
    ...
    echo " - Making gql2php executable...$n";

    @chmod($filename, 0755);

    echo "{$n}gql2php installed!$n";
